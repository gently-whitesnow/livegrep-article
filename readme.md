...
Несколько лет назад в нашей компании начался активный процесс распила монолита на микросервисы. Вскоре появилась необходимость в поиске по нескольким репозиториям одновременно. Эта проблема стала особенно острой, когда количество репозиториев стало расти, и возникла потребность вносить крупные изменения, затрагивающие несколько проектов.
...
8. Ознакомиться со всем кодом проекта можно на GitHub. Его совсем немного, и большая часть уже была представлена в статье.  
**[Ссылка](https://github.com/gently-whitesnow/livegrep-article)**

## Что необходимо еще сделать касаемо безопасности

- Избежать встраивания токена в путь к репозиторию для авторизации — это плохая практика. Вместо этого можно настроить авторизующую прокси, которая будет безопасно обрабатывать аутентификацию.

- Если Livegrep поддерживает SSH-клонирование, можно рассмотреть возможность его использования.

## Заключение

Теперь в русском сегменте есть хоть какой-то опыт запуска этого проекта. Понимаю, что предложенный подход может быть не идеальным, и, вероятно, я не охватил все возможные аспекты, однако основная задача решена.
